...
x_data = np.zeros([xsize, total_size])  # zeros(shape, dtype=float, order='C') :: Return a new array of given shape and type, filled with zeros.

a_true = 2
b_true = 0.5

for i in range(total_size):
    x_data[:, i] = np.linspace(0, 10, xsize)  # linspace :: 0 에서 10 사이에 xsize 만큼의 난수를 생성.

noise_var = 1.0
noise = np.sqrt(noise_var) * np.random.randn(xsize, total_size)  # check
...
with tf.variable_scope(name_or_scope='model', values=[x, y]):
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):  # iteration loop
        ...
        if (epoch + 1) % display_step == 0:
            ...
            error_rate_validation[epoch] = accuracy.eval({x: x_validation_data, y: y_validation_data},
                                                         session=sess) / validation_size
    ...
    print("Validation set Err rate:",
          accuracy.eval({x: x_validation_data, y: y_validation_data}, session=sess) / validation_size)
...
plt.plot(x_validation_data[:, 0], y_validation_data[:, 0], label='noisy data', color='b', marker='o')
