...
optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.7).minimize(cost)
# optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)
# optimizer = tf.train.AdagradDAOptimizer(learning_rate=learning_rate, global_step=1.).minimize(cost)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            _, local_batch_cost = sess.run([optimizer, cost], feed_dict={X: batch_xs, Y: batch_ts})
            
            if epoch == training_epochs - 1:
                # print ('Gradient calculation to see gradient vanishing problem')
                _, grad_wrt_weight_layer1 = sess.run([optimizer, grad_wrt_weight_layer1_tensor], feed_dict={X: batch_xs, Y: batch_ts})
                _, grad_wrt_weight_layer2 = sess.run([optimizer, grad_wrt_weight_layer2_tensor], feed_dict={X: batch_xs, Y: batch_ts})
                _, grad_wrt_weight_layer3 = sess.run([optimizer, grad_wrt_weight_layer3_tensor], feed_dict={X: batch_xs, Y: batch_ts})
                _, grad_wrt_weight_layer4 = sess.run([optimizer, grad_wrt_weight_layer4_tensor], feed_dict={X: batch_xs, Y: batch_ts})
                _, grad_wrt_weight_layer5 = sess.run([optimizer, grad_wrt_weight_layer5_tensor], feed_dict={X: batch_xs, Y: batch_ts})
...
plt.title('MomentumOptimizer')
# plt.title('AdamOptimizer')
# plt.title('AdagradDAOptimizer')
