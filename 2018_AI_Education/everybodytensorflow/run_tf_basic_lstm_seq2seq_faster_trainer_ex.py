...
def get_lstm_dynamic_seq2seq_model(X, scope):
    with tf.name_scope(name=scope, values=[X]):
        layers = [tf.nn.rnn_cell.BasicLSTMCell(num_units=model_config['n_neurons'],
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(n_iteration):
            x_batch, y_batch, tx_train, ty_train = gen_seq_data(shift_sample=model_config['shift_sample'],
                                                                sqe_sample_length=model_config['num_steps'])
            x_test, y_test, tx_test, ty_test = gen_seq_data(shift_sample=model_config['shift_sample'],
                                                            sqe_sample_length=model_config['num_steps'])
