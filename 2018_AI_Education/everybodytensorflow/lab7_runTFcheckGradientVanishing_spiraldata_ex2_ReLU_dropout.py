...
# Construct model
logits = neural_net(X, dropoutrate_io=dropoutrate_io)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            _, local_batch_cost = sess.run([optimizer, cost], feed_dict={X: batch_xs, Y: batch_ts, dropoutrate_io: dropoutrate_in_training})
            
            if epoch == training_epochs - 1:
                # print ('Gradient calculation to see gradient vanishing problem')
                _, grad_wrt_weight_layer1 = sess.run([optimizer, grad_wrt_weight_layer1_tensor], feed_dict={X: batch_xs, Y: batch_ts, dropoutrate_io: dropoutrate_in_training})
                _, grad_wrt_weight_layer2 = sess.run([optimizer, grad_wrt_weight_layer2_tensor], feed_dict={X: batch_xs, Y: batch_ts, dropoutrate_io: dropoutrate_in_training})
                _, grad_wrt_weight_layer3 = sess.run([optimizer, grad_wrt_weight_layer3_tensor], feed_dict={X: batch_xs, Y: batch_ts, dropoutrate_io: dropoutrate_in_training})
                _, grad_wrt_weight_layer4 = sess.run([optimizer, grad_wrt_weight_layer4_tensor], feed_dict={X: batch_xs, Y: batch_ts, dropoutrate_io: dropoutrate_in_training})
                _, grad_wrt_weight_layer5 = sess.run([optimizer, grad_wrt_weight_layer5_tensor], feed_dict={X: batch_xs, Y: batch_ts, dropoutrate_io: dropoutrate_in_training})
        ...
        if display_step == 0:
            ...
            errRateTraining[epoch] = 1.0 - accuracy.eval({X: batch_train_xs, \
                                                          Y: batch_train_ys, \
                                                          dropoutrate_io: 0.0}, session=sess)
