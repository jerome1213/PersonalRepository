...
weight_init_type = 3
#weight_init_fn = tf.random_normal_initializer #1
#weight_init_fn = tf.contrib.layers.xavier_initializer() # 2
weight_init_fn = tf.contrib.layers.variance_scaling_initializer() #3
...
'''
    ...
    '''
weights = {
    'h1': tf.get_variable(name='h1_weight', shape=[num_input, n_hidden_1], initializer=weight_init_fn),
    'h2': tf.get_variable(name='h2_weight', shape=[n_hidden_1, n_hidden_2], initializer=weight_init_fn),
    'h3': tf.get_variable(name='h3_weight', shape=[n_hidden_2, n_hidden_3], initializer=weight_init_fn),
    'h4': tf.get_variable(name='h4_weight', shape=[n_hidden_3, n_hidden_4], initializer=weight_init_fn),
    'h5': tf.get_variable(name='h5_weight', shape=[n_hidden_4, n_hidden_5], initializer=weight_init_fn),
    'out': tf.get_variable(name='out_weight', shape=[n_hidden_5, num_classes], initializer=weight_init_fn)
}
...
'''
    ...
    '''
biases = {
    'b1': tf.get_variable(name='b1_weight', shape=[n_hidden_1], initializer=weight_init_fn),
    'b2': tf.get_variable(name='b2_weight', shape=[n_hidden_2], initializer=weight_init_fn),
    'b3': tf.get_variable(name='b3_weight', shape=[n_hidden_3], initializer=weight_init_fn),
    'b4': tf.get_variable(name='b4_weight', shape=[n_hidden_4], initializer=weight_init_fn),
    'b5': tf.get_variable(name='b5_weight', shape=[n_hidden_5], initializer=weight_init_fn),
    'out': tf.get_variable(name='out_b_weight', shape=[num_classes], initializer=weight_init_fn)
}
...
def neural_net(x, batchnorm_istraining_io, batch_norm_epsilon, batch_norm_decay):
    ...
    #layer_1 = tf.layers.batch_normalization(inputs=layer_1, momentum=batch_norm_decay, center=True, scale=True, epsilon=batch_norm_epsilon, training=batchnorm_istraining_io)
    layer_1 = tf.nn.relu(layer_1)
    
    # Hidden fully connected layer with 7 neurons
    layer_2 = tf.add(tf.matmul(layer_1, weights['h2']), biases['b2'])
    #layer_2 = tf.layers.batch_normalization(inputs=layer_2, momentum=batch_norm_decay, center=True, scale=True, epsilon=batch_norm_epsilon, training=batchnorm_istraining_io)
    layer_2 = tf.nn.relu(layer_2)
    
    # Hidden fully connected layer with 7 neurons
    layer_3 = tf.add(tf.matmul(layer_2, weights['h3']), biases['b3'])
    #layer_3 = tf.layers.batch_normalization(inputs=layer_3, momentum=batch_norm_decay, center=True, scale=True, epsilon=batch_norm_epsilon, training=batchnorm_istraining_io)
    layer_3 = tf.nn.relu(layer_3)
    
    # Hidden fully connected layer with 4 neurons
    layer_4 = tf.add(tf.matmul(layer_3, weights['h4']), biases['b4'])
    #layer_4 = tf.layers.batch_normalization(inputs=layer_4, momentum=batch_norm_decay, center=True, scale=True, epsilon=batch_norm_epsilon, training=batchnorm_istraining_io)
    layer_4 = tf.nn.relu(layer_4)
    
    # Hidden fully connected layer with 4 neurons
    layer_5 = tf.add(tf.matmul(layer_4, weights['h5']), biases['b5'])
    #layer_5 = tf.layers.batch_normalization(inputs=layer_5, momentum=batch_norm_decay, center=True, scale=True, epsilon=batch_norm_epsilon, training=batchnorm_istraining_io)
...
grad_wrt_weight_layer1_tensor = tf.gradients(cost, weights['h1'], name='grad_wrt_weight_layer1')
grad_wrt_weight_layer2_tensor = tf.gradients(cost, weights['h2'], name='grad_wrt_weight_layer2')
grad_wrt_weight_layer3_tensor = tf.gradients(cost, weights['h3'], name='grad_wrt_weight_layer3')
grad_wrt_weight_layer4_tensor = tf.gradients(cost, weights['h4'], name='grad_wrt_weight_layer4')
grad_wrt_weight_layer5_tensor = tf.gradients(cost, weights['h5'], name='grad_wrt_weight_layer5')
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            _, local_batch_cost = sess.run([optimizer, cost], feed_dict={X: batch_xs, Y: batch_ts, batchnorm_istraining_io: True})
            
            if epoch == training_epochs - 1:
                # print ('Gradient calculation to see gradient vanishing problem')
                _, grad_wrt_weight_layer1 = sess.run([optimizer, grad_wrt_weight_layer1_tensor], feed_dict={X: batch_xs, Y: batch_ts, batchnorm_istraining_io: True})
                _, grad_wrt_weight_layer2 = sess.run([optimizer, grad_wrt_weight_layer2_tensor], feed_dict={X: batch_xs, Y: batch_ts, batchnorm_istraining_io: True})
                _, grad_wrt_weight_layer3 = sess.run([optimizer, grad_wrt_weight_layer3_tensor], feed_dict={X: batch_xs, Y: batch_ts, batchnorm_istraining_io: True})
                _, grad_wrt_weight_layer4 = sess.run([optimizer, grad_wrt_weight_layer4_tensor], feed_dict={X: batch_xs, Y: batch_ts, batchnorm_istraining_io: True})
                _, grad_wrt_weight_layer5 = sess.run([optimizer, grad_wrt_weight_layer5_tensor], feed_dict={X: batch_xs, Y: batch_ts, batchnorm_istraining_io: True})
        ...
        if display_step == 0:
            ...
            errRateTraining[epoch] = 1.0 - accuracy.eval({X: batch_train_xs, \
                                                          Y: batch_train_ys, \
                                                          batchnorm_istraining_io: False}, session=sess)
...
# plt.plot(batch_index, grad_wrt_weight_layer2_iter, label='layer2', color='r', marker='*')
...
if weight_init_type == 1:
    plt.title('random_normal_initializer')
elif weight_init_type == 2:
    plt.title('xavier_initializer')
elif weight_init_type == 3:
    plt.title('variance_scaling_initializer')
