...
logits = neural_net(X)
prediction = tf.nn.softmax(logits)  # softmax :: 여러개의 클래스의 값을 0~1 의 값으로 만들어 주고 그 합을 1
...
# Evaluate model
correct_pred = tf.equal(tf.argmax(prediction, 1), tf.argmax(Y, 1))  # prediction / Y 각각 제일 큰 값이 같은지 확인
